...
# Computes the mean line; it receives the set of slopes(m) and a set of point where each line passes
def mean_line(m_left, xl, m_right, xr):
    mean_ml = 0
    mean_xl = 0
    mean_mr = 0
    mean_xr = 0
    ...
    if len(m_left) > 0:
        mean_ml = sum(m_left)/len(m_left)
        mean_xl = sum(xl)/len(xl)
    if len(m_right) > 0:
        mean_mr = sum(m_right)/len(m_right)
        mean_xr = sum(xr)/len(xr)

    return mean_ml, mean_xl, mean_mr, mean_xr
    
# Computes the extrema points of a set of lines
def points_of_line(m, x1, h_image):
...
# Draws the lines; main procedure
def process_image(image):    
    ...
    if len(m_right)>0:
           x1,y1,x2,y2 = points_of_line(mrm,xrm,imshape[0])
           cv2.line(image2show,(x1,y1),(x2,y2),(255,255,0),3)
